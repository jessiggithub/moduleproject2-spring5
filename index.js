...
function moduleProject2() {
  ...
  document.addEventListener('keydown', evt => {
   ...
   let targeted= document.querySelector('.targeted');
    ...
    // 👉 TASK 5 - End the game 👈
    let allMosquitoes = document.querySelectorAll('[data-status=alive]');
    if (!allMosquitoes.length) {
      let timeElapsed = getTimeElapsed();
      document.querySelector('p .info').textContent = `Game over! Time elapsed: ${timeElapsed}mms`;
